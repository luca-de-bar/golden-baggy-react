    ...
    <div className="relative w-full">
      <div className="relative h-64 sm:h-96 lg:h-128 overflow-hidden flex items-center justify-center">
        {images.map((image, index) => (
          ...
          >
            <img src={image} alt={`carousel slide ${index}`} className="w-full h-full object-cover lg:object-center-top" />
          </div>
        ))}
      </div>
      <button onClick={prevSlide} className="absolute top-1/2 left-0 transform -translate-y-1/2 bg-black bg-opacity-50 text-white px-4 py-2 rounded-r-lg focus:outline-none">
        &#10094;
      </button>
      <button onClick={nextSlide} className="absolute top-1/2 right-0 transform -translate-y-1/2 bg-black bg-opacity-50 text-white px-4 py-2 rounded-l-lg focus:outline-none">
        &#10095;
      </button>
      <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 flex space-x-1 pb-4">
        {images.map((_, index) => (
          <div key={index} className={`w-2.5 h-2.5 rounded-full ${index === currentIndex ? "bg-white" : "bg-gray-400"}`} />
        ))}
      </div>
    </div>
  );
